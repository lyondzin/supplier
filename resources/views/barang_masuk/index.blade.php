...
<div class="max-w-[760px] mx-auto py-32 sm:py-8 lg:py-16">
  ...
  <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
      <div>
          ...
          <p class="text-slate-500">A list of all barang masuk..</p>
      </div>
      <div class="ml-3">
  ...
  <table class="w-full text-left table-auto min-w-max">
      ...
      <tr class="hover:bg-slate-50 border-b border-slate-200">
          ...
          <td class="p-4 py-5">
            <p class="text-sm text-slate-500">{{ $barang_masuk->barang->nama_barang }}</p>
            </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $barang_masuk->tgl_masuk }}</p>
          </td>
          <td class="p-4 py-5">
          ...
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500"><a href="{{ route('barang_masuk.edit', $barang_masuk->id_barang_masuk) }}" class="font-medium text-blue-600 hover:text-blue-800">Edit</a></p>
          </td>
          <td class="p-4 py-5">
           <p class="text-sm text-slate-500">
               <a href="{{ route('barang_masuk.destroy', $barang_masuk->id_barang_masuk) }}"
                   class="font-medium text-blue-600 hover:text-blue-800"
                   onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this barang masuk?')) { document.getElementById('delete-form-{{ $barang_masuk->id_barang_masuk }}').submit(); }">
                    Hapus
                </a>
                <form id="delete-form-{{ $barang_masuk->id_barang_masuk }}" action="{{ route('barang_masuk.destroy', $barang_masuk->id_barang_masuk) }}" method="POST" style="display: none;">
                   @csrf
                   @method('DELETE')
               </form>
           </p>
